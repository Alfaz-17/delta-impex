...
function ProductFormContent() {
  ...
  const fetchDivisions = async () => {
    try {
      ...
      if (!activeDivisionId && data.length > 0) {
        setFormData(prev => ({ ...prev, division: data[0]._id }));
        fetchCategories(data[0]._id);
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name || !formData.division || !formData.category || !formData.imageUrl) {
      toast.error("System Error: Please complete all required technical fields.");
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        toast.success(editingId ? "Inventory updated successfully" : "New entry registered to database");
        router.push(`/admin/products${activeDivisionId ? "?divisionId=" + activeDivisionId : ""}`);
      } else {
        toast.error("Database connection refused.");
      }
    } catch (error) {
      toast.error("Runtime exception occurred.");
    } finally {
      setIsLoading(false);
    }
  };
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                    ...
                    {formData.imageUrl ? (
                      ...
                      <label className="flex flex-col items-center justify-center aspect-video max-w-sm border-2 border-dashed border-border bg-muted/20 hover:bg-muted/40 transition-colors cursor-pointer group">
                        ...
                        <span className="mt-4 text-[10px] font-bold uppercase tracking-widest text-primary">
                          {uploading ? "Syncing to Cloud..." : "Upload Graphics"}
                        </span>
                ...
                <div className="flex items-center justify-between p-6 border border-border bg-muted/20 mt-8">
                  <div className="flex items-center gap-3">
                    <Star className={`w-5 h-5 ${formData.isFeatured ? "text-accent fill-accent" : "text-muted-foreground"}`} />
                    <div>
                        <Label className="text-[10px] font-bold uppercase tracking-widest block text-primary">Primary Feature Status</Label>
                        <p className="text-[9px] text-muted-foreground uppercase mt-1">Display this unit on the main overview screens.</p>
                    </div>
                  </div>
                  <Switch 
                ...
                <div className="pt-8 flex gap-4">
                    ...
                    >
                        {isLoading ? "Writing to Database..." : editingId ? "Commit Updates" : "Finalize Registration"}
                    </Button>
                </div>
              </form>
            </div>
...
export default function AdminProductFormPage() {
  return (
    <Suspense fallback={<div className="h-screen flex items-center justify-center bg-muted/30"><Loader2 className="w-8 h-8 animate-spin text-primary" /></div>}>
      <ProductFormContent />
    </Suspense>
  );
}
