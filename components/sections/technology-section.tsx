      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-20 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-20 items-center">
          ...
          <div className="order-2 lg:order-1">
            <FadeInOnScroll>
              <p className="label-tech text-accent mb-6">{label || "Advanced Engineering"}</p>
              <h2 className="heading-display text-primary mb-8 uppercase tracking-tighter leading-none">
                {heading ? heading.split('\n').map((line, i) => (
                  <span key={i}>{line}<br /></span>
                )) : (
                  <>RO Water <br /><span className="text-accent italic font-medium">Desalination.</span></>
                )}
              </h2>
              <div className="space-y-8 max-w-xl">
                <p className="body-premium text-slate-600">
                  {description || "We supply specialized RO systems for both marine and industrial use, designed to convert saline seawater into clean, safe, and potable freshwater."}
                </p>
                <p className="body-text text-slate-500 text-sm leading-relaxed">
                  {secondaryDescription || "Our technology plays a vital role in regions where freshwater resources are limited, especially in coastal and industrial areas. We provide complete solutions including Reverse Osmosis plants, water treatment equipment, and specialized maintenance support."}
                </p>
            ...
            <FadeInOnScroll direction="right">
              <div className="relative group">
                ...
                <div className="relative aspect-square md:aspect-[4/5] lg:aspect-square overflow-hidden rounded-none border border-primary/5 shadow-2xl">
                  <Image
                    src={image || "/ro/ro-plant-framed.png"}
                ...
                <div className="absolute -bottom-6 -left-6 bg-primary p-6 border border-white/10 hidden md:block">
                  <p className="font-tech text-[9px] uppercase tracking-[0.3em] text-white/60 mb-2">{badgeLabel || "Technical Standard"}</p>
                  <p className="font-display font-bold text-xl text-white">{badgeTitle || "ISO CERTIFIED"}</p>
                </div>
              </div>
            </FadeInOnScroll>
          </div>
